...
data_dir = '../data_clipped'
files = os.listdir(data_dir)
files = [ files[i] for i in range(4, 38, 8)]
...
for f in files:
    ...
    speed = decoded_classes[1]

    plt.subplot(len(files), 1, files.index(f)+1)
    timeline = np.linspace(0, len(data) / float(fs), len(data))
    plt.title('Distance %d cm - Speed %d $^\circ$/s' % (distance, speed))
