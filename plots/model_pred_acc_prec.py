...
f_hi = model_scores.loc[:, 'freqhi_actual']
f_lo = model_scores.loc[:, 'freqlo_actual']
r2 = model_scores.loc[:, 'r2']

sorted_r2 = sorted(r2, reverse=True)
sorted_f_hi = [ r for _, r in sorted(zip(r2, f_hi), reverse=True) ]
sorted_f_lo = [ r for _, r in sorted(zip(r2, f_lo), reverse=True) ]

TRAIN_ON = [sorted_f_lo[-1], sorted_f_hi[1]]
